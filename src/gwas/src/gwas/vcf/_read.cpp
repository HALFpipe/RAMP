...
struct RingBuffer {
  ...
  size_t push_from_pipe(const int file_descriptor) {
    ...
    // std::cout << "push_from_pipe write_loop_count " << write_loop_count <<
    // std::endl; std::cout << "push_from_pipe loop_count " << loop_count <<
    // std::endl;
    ...
    // std::cout << "push_from_pipe index " << index << std::endl;
    // std::cout << "push_from_pipe actual_write_index " << actual_write_index
    // << std::endl; std::cout << "push_from_pipe actual_index " << actual_index
    // << std::endl; std::cout << "push_from_pipe data_.size " << data_.size()
    // << std::endl; std::cout << "push_from_pipe read_size " << read_size <<
    // std::endl;
    ...
    // std::cout << "push_from_pipe data_ \"" << std::string_view(data_.data() +
    // actual_write_index, bytes_read) << "\"" << std::endl;
  ...
  __attribute__((target("avx2"))) __m256i get_m256i(const size_t index) {
    ...
    if (actual_first_index < actual_last_index) [[likely]] {
      ...
      //           << "\"" << std::string_view(data_.data() +
      //           actual_first_index, sizeof(__m256i)) << "\""
      //           << std::endl;
  ...
  std::string_view get_token(const size_t begin_index, const size_t end_index) {
    ...
    if (actual_begin_index < actual_end_index) [[likely]] {
      ...
      // std::cout << "wrap_token"
      //           << " \"" << std::string_view(data_.begin() +
      //           actual_begin_index, data_.end()) << "\""
      //           << " \"" << std::string_view(data_.begin(), data_.begin() +
      //           actual_end_index) << "\""
      //           << " \"" << std::string_view(value_buffer_.data(),
      //           value_buffer_.size()) << "\""
      //           << std::endl;
    ...
    //           << " actual_begin_index " <<
    //           static_cast<int>(actual_begin_index)
    //           << " actual_end_index " << static_cast<int>(actual_end_index)
    //           << " value_buffer_.size() " <<
    //           static_cast<int>(value_buffer_.size())
  ...
  RingBuffer(size_t size) : data_(size) {
    if (size % page_size != 0) {
      // std::cerr << "`RingBuffer` size must be a multiple of page size" <<
      // std::endl;
      PyErr_SetString(PyExc_ValueError,
                      "`RingBuffer` size must be a multiple of page size");
      throw error_already_set();
    }
  }
...
template <class Parser> struct Reader : RingBuffer {
  ...
  Reader(int file_descriptor, size_t skip_bytes, size_t column_count,
      ...
      : RingBuffer(ring_buffer_size), file_descriptor_(file_descriptor),
        skip_bytes_(skip_bytes), column_count_(column_count),
  ...
  void remainder_loop(size_t &start) {
    ...
    for (; read_index_ < write_index_; read_index_++) {
      ...
      if (is_delimiter) {
        reinterpret_cast<Parser *>(this)->on_token(
    ...
    while (push_from_pipe(file_descriptor_)) {
      ...
      for (; read_index_ + sizeof(__m128i) - 1 < write_index_;
        ...
        while (is_delimiter) {
          ...
          bool continue_reading = reinterpret_cast<Parser *>(this)->on_token(
  ...
  __attribute__((target("avx2,bmi"))) void avx2_bmi_loop() {
    ...
    remainder_loop(start);
    // std::cout << "avx2_bmi_loop read_index_ " << read_index_ << "
    // write_index_ " << write_index_ << " start " << start << std::endl;
  }
};
...
struct StrReader : public Reader<StrReader> {
  ...
  bool on_token(std::string_view token) {
    ...
    //           static_cast<int>(column_indices_index_)
    //           << " column_indices_[column_indices_index_] " <<
    //           static_cast<int>(column_indices_[column_indices_index_])
    //           << " " << token << std::endl;
  ...
  StrReader(PyObject *rows, PyObject *parser, int file_descriptor,
            size_t skip_bytes, size_t column_count, uint32_t *column_indices,
...
struct FloatReader : public Reader<FloatReader> {
  ...
  bool on_token(std::string_view token) {
    ...
    //     row_indices_[row_indices_index_],
    //     column_indices_[column_indices_index_], record_str, column_count_);
    ...
    if (row_indices_[row_indices_index_] == current_row_index) {
      if (column_indices_[column_indices_index_] == current_column_index) {
        ...
        size_t float_index =
            column_indices_index_ + row_indices_index_ * column_index_count_;
        if (std::from_chars(begin, token.end(), float_array_[float_index]).ec !=
            std::errc{}) {
        ...
        // py::print("from_chars", column_indices_index_, row_indices_index_,
        // column_index_count_, float_index, float_array_[float_index]);
  ...
  FloatReader(double *float_array, int file_descriptor, size_t skip_bytes,
      ...
      : Reader(file_descriptor, skip_bytes, column_count, column_indices,
               column_index_count, ring_buffer_size),
        float_array_(float_array), field_index_(field_index),
...
static PyObject *ReadStr(PyObject * /* self */, PyObject *arguments,
  ...
  if (!PyArg_ParseTuple(arguments, "O!OikIO!", &(PyList_Type), &row_list, // O!
                        &row_parser,                                      // O
                        &file_descriptor,                                 // i
                        &skip_bytes,                                      // k
                        &column_count,                                    // I
  ...
  uint32_t *column_indices =
      reinterpret_cast<uint32_t *>(PyArray_DATA(column_indices_array));
...
PyCapsule_Destructor DestroyFloatReader = [](PyObject *float_reader_capsule) {
  ...
  FloatReader *float_reader = (FloatReader *)PyCapsule_GetPointer(
      float_reader_capsule, FloatReader::name);
  if (float_reader) {
    delete float_reader;
  }
};

static PyObject *CreateFloatReader(PyObject * /* self */, PyObject *arguments,
                                   Py_ssize_t argument_count) {
  int file_descriptor = 0;
  unsigned long skip_bytes = 0;
  unsigned int column_count = 0;
  ...
  if (!PyArg_ParseTuple(arguments, "ikIO!I",
                        &file_descriptor,                       // i
                        &skip_bytes,                            // k
                        &column_count,                          // I
                        &(PyArray_Type), &column_indices_array, // O!
  ...
  PyObject *float_reader_capsule =
      PyCapsule_New(float_reader, FloatReader::name, DestroyFloatReader);
...
static PyObject *RunFloatReader(PyObject * /* self */, PyObject *arguments,
                                Py_ssize_t argument_count) {
  PyArrayObject *data_array;
  PyObject *float_reader_capsule;
  PyArrayObject *row_indices_array;
  if (!PyArg_ParseTuple(arguments, "O!O!O!", &(PyArray_Type), &data_array, // O!
  ...
  FloatReader *float_reader =
      (FloatReader *)PyCapsule_GetPointer(float_reader_capsule, name);
  ...
  if (PyArray_TYPE(data_array) != NPY_DOUBLE) {
    PyErr_SetString(PyExc_ValueError,
                    "`data_array` must be an array of doubles");
    return nullptr;
  }
  ...
  if (PyArray_STRIDES(data_array)[0] != static_cast<npy_intp>(sizeof(double)) ||
      PyArray_STRIDES(data_array)[1] !=
          static_cast<npy_intp>(sizeof(double) * column_index_count)) {
    PyErr_SetString(
        PyExc_ValueError,
        "The strides of `data_array` are not in column-major order");
    return nullptr;
  }

  size_t row_index_count = PyArray_SIZE(row_indices_array);
  uint32_t *row_indices =
      reinterpret_cast<uint32_t *>(PyArray_DATA(row_indices_array));
  if (!CheckIndexArray(row_indices_array)) {
    return nullptr;
  }
  if (PyArray_SHAPE(data_array)[0] !=
          static_cast<npy_intp>(column_index_count) ||
      PyArray_SHAPE(data_array)[1] != static_cast<npy_intp>(row_index_count)) {
    PyErr_SetString(PyExc_ValueError,
                    "The shape of `data_array` does not match the shape of the "
