...
cdef extern from "htslib/vcf.h":
    ...
    ctypedef struct bcf_info_t:
        ...
        uv1 v1
        uint8_t *vptr          # pointer to data array in bcf1_t->shared.s,
        # excluding the size+type and tag id bytes
        uint32_t vptr_len      # length of the vptr block or, when set,
        # of the vptr_mod block, excluding offset
        uint32_t vptr_off
        uint32_t vptr_free   # vptr offset, i.e., size of INFO key plus size+type bytes
        int len  # indicates vptr-vptr_off must be freed; set only when modified and new

    ctypedef struct bcf_dec_t:
        int m_fmt, m_info, m_id, m_als, m_allele, m_flt  # allocated size; dont change
    ...
    ctypedef struct bcf1_t:
        ...
        bcf_dec_t d  # lazy eval: $d not generated by bcf_read(), but by bcf_unpack()
        int max_unpack
        int unpacked
    ...
    ctypedef struct bcf_hrec_t:
        ...
        char *key      # Part before '=', i.e. FILTER/INFO/FORMAT/contig/fileformat etc
