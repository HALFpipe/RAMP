...
def find_phenotype_index(phenotype_label: str, phenotypes_list: List) -> Tuple[int, int]:
    ...
    if u_stat_idx is not None and v_stat_idx is not None:
        ...
        raise ValueError(f"Matching indices for '{phenotype_label}' not found. u_stat_idx: {u_stat_idx}, v_stat_idx: {v_stat_idx}")
...
def save_dataframe_as_pickle(dataframe: pd.DataFrame, directory: Path, label: str) -> None:
...
def generate_and_save_manhattan_plot(dataframe: pd.DataFrame, directory: Path,label: str) -> None:
    ...
    ax = manhattanplot(data=dataframe, genomewideline=1e-8, chrom='CHR', pos='BP', pv='P', snp='SNP', xticklabel_kws={"rotation": "vertical"})
