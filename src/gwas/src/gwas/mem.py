...
def limit_from_file(file_name: Path) -> int | None:
    try:
...
def cgroup_memory_limit() -> int | None:
    ...
    cg_infos: dict[str, dict[str, Any]] = dict(
        memory=dict(), cpuset=dict(), cpu=dict(), cpuacct=dict()
    )

    # Read /proc/cgroups so as to be able to distinguish
    # cgroups v2 vs cgroups v1
    try:
    ...
    for p in cgroups_lines:
        m = re.fullmatch(
            r"(?P<name>\w+)\s(?P<id>\d+)\s\d+\s(?P<enabled>\d+)",
            p.strip(),
        )
        if m is None:
            continue
        name = str(m.group("name"))
        hierarchy_id = int(m.group("id"))
    ...
    for p in cgroup_lines:
        m = re.fullmatch(
            r"(?P<id>\d+):(?P<controllers>[^:]*):(?P<cgroup_path>.+)",
            p.strip(),
        )
        if m is None:
            continue
    ...
    for p in mountinfo_lines:
        m = re.fullmatch(
            r"\d+\s\d+\s\d+:\d+\s(?P<root>[^\s]+)\s"
            r"(?P<mount_point>[^\s]+)\s[^-]+-\s"
            r"(?P<fs_type>[^\s]+)\s[^\s]+\s(?P<cgroups>[^\s]+)",
            p.strip(),
        )
        if m is None:
            continue
    ...
    if all(len(cg_info) == 0 for cg_info in cg_infos):
        # Neither cgroup2 nor cgroup filesystems mounted
        # via /proc/self/mountinfo. No point in continuing.
        return None
    ...
    memory_limits = set()
    for q in memory_cgroups_to_consider:
        for f in memory_limit_files:
            memory_file_name = q / f
...
def ulimit_memory_limit() -> int | None:
    try:
        proc = subprocess.Popen(
            ["bash", "-c", "ulimit -a"],
            # make the process its own process group
...
def available_memory_bytes() -> int | None:
    ...
    try:
        with open(proc_meminfo, "r") as meminfo_file_pointer:
            meminfo_lines = meminfo_file_pointer.readlines()
    except (OSError, IOError):
        return None
    ...
    for p in meminfo_lines:
        ...
        meminfo[m.group("id")] = int(m.group(("value"))) * factor
