...
protected:
          ...
          file_descriptor_(file_descriptor), skip_bytes_(skip_bytes)
    {
        ...
        if (column_count_ == 0)
        {
            throw std::invalid_argument("`column_count` must be greater than 0");
        }
        if (column_index_count_ == 0)
        {
            throw std::invalid_argument("`column_index_count` must be greater than 0");
        }
        if (ring_buffer_size % page_size != 0)
        {
            throw std::invalid_argument("`ring_buffer_size` must be a multiple of page size");
        }
    }
