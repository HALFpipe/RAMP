...
static PyObject *ReadFloat(PyObject *self, PyObject *args, PyObject *kwargs)
{
  PyArrayObject *array;
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O!ikIO!O!|$I", const_cast<char **>(keywords),
  ...
  try
  {
    TSVFloatReader reader{
        array,
        file_descriptor,
        static_cast<size_t>(skip_bytes), static_cast<size_t>(column_count),
        column_indices, column_index_count,
        row_indices, row_index_count,
...
{
  if (!PyCapsule_IsValid(float_reader_capsule, VCFFloatReader::name))
  {
    return;
  }

  VCFFloatReader *vcf_float_reader = (VCFFloatReader *)PyCapsule_GetPointer(
      float_reader_capsule, VCFFloatReader::name);
  if (vcf_float_reader)
  {
    delete vcf_float_reader;
  }
};
...
static PyObject *CreateVCFFloatReader(PyObject *self, PyObject *args, PyObject *kwargs)
{
  ...
  VCFFloatReader *vcf_float_reader = nullptr;
  try
  {
    vcf_float_reader = new VCFFloatReader(
  ...
  PyObject *capsule =
      PyCapsule_New(vcf_float_reader, VCFFloatReader::name, DestroyFloatReader);
...
static PyObject *RunVCFFloatReader(PyObject * /* self */, PyObject *arguments,
                                   Py_ssize_t argument_count)
{
  PyArrayObject *array;
  PyObject *float_reader_capsule;
  PyArrayObject *row_indices_array;
  if (!PyArg_ParseTuple(arguments, "O!O!O!", &(PyArray_Type), &array, // O!
  ...
  VCFFloatReader *vcf_float_reader =
      (VCFFloatReader *)PyCapsule_GetPointer(float_reader_capsule, name);
