...
const size_t level1_dcache_size = sysconf(_SC_LEVEL1_DCACHE_SIZE);
...
size_t RingBuffer::push_from_pipe(const int file_descriptor)
{
...
char RingBuffer::get_byte(const size_t index)
{
    size_t actual_index = index % data_.size();
    return data_[actual_index];
}

__m128i RingBuffer::get_m128i(const size_t index)
{
...
__m256i RingBuffer::get_m256i(const size_t index)
{
    size_t first_index = index;
...
std::string_view RingBuffer::get_token(const size_t begin_index, const size_t end_index)
{
